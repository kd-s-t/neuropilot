...
class DJICommands:
    """Map control IDs to DJI Tello commands"""
    
    # Exact control_id (lowercase) -> Tello command. down/stop/land handled by early exit.
    ...
    # Default movement distances/angles
    DEFAULT_DISTANCE = 20  # cm
    DEFAULT_ANGLE = 90  # degrees
    
    @staticmethod
    def get_command(control_id: str, value: Optional[int] = None) -> Optional[str]:
        """
        Convert control ID to DJI Tello command
        
        Args:
            control_id: The control identifier (e.g., "forward", "takeoff")
            value: Optional value for commands that need parameters (distance in cm or angle in degrees)
        
        Returns:
            Command string or None if control_id not recognized
        """
        control_lower = (control_id or "").strip().lower()
        # Down = descend (cm). Stop/Land = land. Never treat "down" as land.
        ...
        if control_lower in DJICommands.DEFAULT_MAPPINGS:
            ...
            if base_command in ["forward", "back", "left", "right", "up", "down"]:
                ...
                # No-param commands: takeoff, land, flip l/r/f/b, emergency, streamon, streamoff
                return base_command
        
        # Try partial matches for flexibility (check turn/rotate before left/right so "turn left" -> ccw, not left)
